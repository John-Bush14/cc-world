...
print("Parsing song!")
local songFile = "ipod/Undertale.nbs"
...
while true do 
    ...
    while type(note) == "table" and not paused do
        ...
        speaker.playNote(instruments[note.instrument + 1] or "guitar", note.velocity/(10/3), math.min(math.max((note.key-33)+(note.pitch/100), 0), 24))
        k, note = next(song.notes, k)
        print(os.clock()-startTick)
    end
